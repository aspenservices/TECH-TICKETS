...
const CACHE_VERSION = "v1.15.0";  // 23 jul 2026 — v1.15.0 = reagendar: la nota lleva su fecha, la linea de tiempo se ordena por fecha, y franja de reagendar arriba del ticket. v1.14.0 = logica BluFusion corregida (con circ -> BF/CP en el sitio del ozono; sin circ -> compartido con Pump 1) + config 14 lleva O3 no Pump 5. v1.13.1 = fix aterrizaje del Viewer (caia en calendario) + etiqueta de rol. v1.13.0 = rol Viewer (solo Tech Tools > YE-6, sin listeners de Firestore). v1.12.0 = YE-6 rediseno (paleta Aspen, color por terminal, fix A6, fixes movil) + BluFusion. mismo numero que APP_VERSION, bump juntos. v1.11.0 = Cover Tracker → Service Log bridge (structured deep-link intake, idempotent by srcId, cleanup-exempt). v1.10.0 = fases 1-3 auditoria.
